...
from perceiver import dataset
from perceiver import losses
from perceiver import perceiver_ar_model
...
def get_config(arg_string):
    ...
    config.save_checkpoint_interval = 1000  # just for in-memory checkpointing.
...
def get_sweep(sweep_name, train_device_count, eval_device_count):
    ...
    elif sweep_name == 'rulm_test':
        sweep_parameters = [
            ('dataset_loader', 'rulm_test'),
            ('max_context_length', 4096),
            (f'{MODEL_KWARGS}.max_wavelength', 4096),

            # ----- Start active sweep ------
            (f'{EXP_CONFIG}.data.minimum_crop_length', 2048),
            (f'{MODEL_KWARGS}.cross_attention_head_group_size', 4),
            (f'{MODEL_KWARGS}.z_index_dim', 1024),
            (f'{MODEL_KWARGS}.position_encoding_type', 'absolute'),
            (f'{MODEL_KWARGS}.position_encoding', 'sinusoidal'),
...
class Experiment(experiment.AbstractExperiment):
    ...
    def __init__(self, mode, init_rng, config):
        ...
        wandb.init(project="text-perceiver")
