...
class RuLMTestDataset(Dataset):
    """C4 dataset."""

    def __init__(self, sequence_length: int):
        self._sequence_length = sequence_length
        self._tokenizer = AutoTokenizer.from_pretrained("ai-forever/rugpt3large_based_on_gpt2")
    ...
    def load(self, split: Split, is_training: bool, include_sos: bool):
        ...
        def tokenize(text):
            return self._tokenizer(
                ...
                padding='max_length',
                return_tensors='tf',
                add_special_tokens=include_sos)

        def process_example(example):
            tokenized = tokenize(example['text'])
            return {'events': tokenized.input_ids[0],
                    'event_idxs': tokenized.attention_mask[0]}

        ds = datasets.load_dataset('IlyaGusev/rulm', split=hf_split)
        ds = ds.map(
            process_example,
            batched=True,
            batch_size=32,  # Tune this based on tokenizer performance.
            num_proc=AUTOTUNE)
...
DATASET_LOADERS = {
    ...
    'dummy': DummyDataset(),
    'rulm_test': RuLMTestDataset(sequence_length=4096),
